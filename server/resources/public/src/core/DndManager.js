class DndManager {
  ...
  onDragOver(e) {
    ...
    // Проверяем все возможные цели для drop
    ...
    if ($row.length || $limit.length || $personnel.length) {
      ...
      // Подсвечиваем строку, если курсор не над конкретным блоком
  ...
  onDrop(e) {
    ...
    if ($access.length) {
      const zoneIndex = parseInt($access.attr('data-group-index'), 10);
      if (!isNaN(zoneIndex)) {
        this.model.updateWorkAreaGroupApplic?.(rowIndex, zoneIndex, applicId);
        return this.emitChange({ type: 'applic:dropped', payload: { rowIndex, zoneIndex, applicId, target: 'zone' } });
      }
    ...
    if ($remarks.length){
          if (this.model.updateRemarksApplic) {
              this.model.updateRemarksApplic(rowIndex, applicId);
              return this.emitChange({ type: 'applic:dropped', payload: { rowIndex, personnelIndex, applicId, target: 'remarks' } });
          }
    }

    // 3. Проверяем field cell
    ...
    // 4. Fallback: whole task
    const taskId = $row.attr('data-task-id') ?? rowIndex;
    if (this.model.setApplicability) {
      this.model.setApplicability(taskId, applicId);
    } else {
      this.model.updateApplicForTask?.(rowIndex, applicId);
    }
